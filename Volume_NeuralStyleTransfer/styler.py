...
parser.add_argument("--style_path", type=str, default='D:\\Review\\vnst\\Houdini-Plugin-for-Tensorflow-Smoke-Stylization-master')

parser.add_argument("--data_dir", type=str, default='D:\\Review\\vnst\\Houdini-Plugin-for-Tensorflow-Smoke-Stylization-master\\data\\smoke_gun')
parser.add_argument("--log_dir", type=str, default='D:\\Review\\vnst\\Houdini-Plugin-for-Tensorflow-Smoke-Stylization-master\\data\\smoke_gun\\r')
parser.add_argument("--npz2vdb_dir", type=str, default='D:\\Review\\vnst\\Houdini-Plugin-for-Tensorflow-Smoke-Stylization-master\\data\\data\\npz2vdb')
parser.add_argument("--tag", type=str, default='test')
parser.add_argument("--seed", type=int, default=777)
parser.add_argument("--model_path", type=str, default='data\\model\\tensorflow_inception_graph.pb')
...
parser.add_argument("--content_target", type=str, default='D:\\Review\\vnst\\Houdini-Plugin-for-Tensorflow-Smoke-Stylization-master\\data\\image\\fire.png')
parser.add_argument("--style_target", type=str, default='D:\\Review\\vnst\\Houdini-Plugin-for-Tensorflow-Smoke-Stylization-master\\data\\image\\fire.png')
...
def stylize(args):
    ...
    if (not args.houdini): 
        ...
        for i in trange(args.num_frames):
            ...
            with np.load(d_path) as data:
                print("文件中的可用键:", data.files)  # 打印所有可用的键
                #d_ = data['x'][:,::-1]
                d_ = data['d'][:, ::-1]
    ...
    if (args.houdini):
        ...
        for i, d_sty_ in enumerate(d_sty):
            ...
            transmit = np.exp(-np.cumsum(d_sty_[::-1], axis=0)*args.transmit)
            d_sty_img = np.sum(d_sty_[::-1]*transmit, axis=0)
            print("d_sty_img: ", d_sty_img.max(), d_sty_img.min())
            d_sty_img /= d_sty_img.max()
            im = d_sty_img*255
            im = Image.fromarray(im.astype(np.uint8))
